...
import type { Wartungsplan, Maschinen, CreateWartungsplan, CreateMaschinen } from '@/types/app';
...
export class LivingAppsService {
  // --- WARTUNGSPLAN ---
  static async getWartungsplan(): Promise<Wartungsplan[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGSPLAN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Wartungsplan[];
    return enrichLookupFields(records, 'wartungsplan');
  }
  static async getWartungsplanEntry(id: string): Promise<Wartungsplan | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGSPLAN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Wartungsplan;
    return enrichLookupFields([record], 'wartungsplan')[0];
  }
  static async createWartungsplanEntry(fields: CreateWartungsplan) {
    return callApi('POST', `/apps/${APP_IDS.WARTUNGSPLAN}/records`, { fields: cleanFieldsForApi(fields as any, 'wartungsplan') });
  }
  static async updateWartungsplanEntry(id: string, fields: Partial<CreateWartungsplan>) {
    return callApi('PATCH', `/apps/${APP_IDS.WARTUNGSPLAN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'wartungsplan') });
  }
  static async deleteWartungsplanEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WARTUNGSPLAN}/records/${id}`);
  }

  // --- MASCHINEN ---
  static async getMaschinen(): Promise<Maschinen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MASCHINEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Maschinen[];
    return enrichLookupFields(records, 'maschinen');
  }
  static async getMaschinenEntry(id: string): Promise<Maschinen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MASCHINEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Maschinen;
    return enrichLookupFields([record], 'maschinen')[0];
  }
  static async createMaschinenEntry(fields: CreateMaschinen) {
    return callApi('POST', `/apps/${APP_IDS.MASCHINEN}/records`, { fields: cleanFieldsForApi(fields as any, 'maschinen') });
  }
  static async updateMaschinenEntry(id: string, fields: Partial<CreateMaschinen>) {
    return callApi('PATCH', `/apps/${APP_IDS.MASCHINEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'maschinen') });
  }
  static async deleteMaschinenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MASCHINEN}/records/${id}`);
  }
