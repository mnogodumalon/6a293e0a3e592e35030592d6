...
**`StatCard`** — full API: `title, value, description?, icon?, tone?, onClick?, active?, footer?, className?`. `icon` must be rendered JSX, NOT a component reference:
```tsx
...
```
KPI rules: a KPI earns its place only if someone would ACT on its value — drop filler stats ("Einträge gesamt: 4"); 2–4 cards typical, never pad to 4. A clickable KPI filters the surface below it (toggle off on second click) — do NOT also render a duplicate filter-chip row for the same dimension. `footer` is for real context only (delta, mini sparkline via recharts, progress toward a target, next deadline) — omit it when the data has none.
...
**One message per dashboard — the critical state gets the hero.** When the data carries an urgent signal (something overdue, unstaffed, over capacity), give it the MOST prominent surface: a full-width alert banner with the concrete facts and a direct action button (`"1 Wartung 26 Tage überfällig — [Jetzt planen]"`), placed above everything else. Do NOT bury the one thing the user must act on inside one of four equally-sized boxes. KPIs are the secondary line under the hero; when nothing is urgent, the primary widget itself is the hero and no banner renders.
